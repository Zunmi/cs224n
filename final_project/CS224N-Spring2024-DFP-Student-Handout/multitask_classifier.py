...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        pair_feature_size = 3 * BERT_HIDDEN_SIZE
        self.paraphrase_classifier = nn.Linear(pair_feature_size, 1)
        self.similarity_regressor = nn.Linear(pair_feature_size, 1)
    ...
    def predict_paraphrase(self,
        ...
        pair_features = torch.cat(
            [embedding_1, embedding_2, torch.abs(embedding_1 - embedding_2)], dim=-1
        )
        return self.paraphrase_classifier(pair_features).squeeze(-1)
    ...
    def predict_similarity(self,
        ...
        return self.similarity_regressor(pair_features).squeeze(-1)
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(para_train_dataloader, desc=f'train-para-{epoch}', disable=TQDM_DISABLE):
            ...
            loss = F.binary_cross_entropy_with_logits(logits, b_labels, reduction='mean')
            loss.backward()
            optimizer.step()
        ...
        dev_acc, dev_sst_pred, dev_sst_ids, dev_para_acc, dev_para_pred, dev_para_ids, dev_sts_corr, dev_sts_pred, dev_sts_ids = \
            model_eval_multitask(sst_dev_dataloader, para_dev_dataloader, sts_dev_dataloader, model, device)

        dev_score = (dev_acc + dev_para_acc + dev_sts_corr) / 3.0
        ...
        print(
            f"Epoch {epoch}: train loss :: {train_loss :.3f}, train sst acc :: {train_acc :.3f}, "
            f"dev sst acc :: {dev_acc :.3f}, dev para acc :: {dev_para_acc :.3f}, dev sts corr :: {dev_sts_corr :.3f}"
        )
