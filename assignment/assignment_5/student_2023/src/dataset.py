...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(doc) < 4: 
            ...
            trunc_len = random.randint(4, min(len(doc), max_len))
        doc = doc[:trunc_len]

        if len(doc) == 0:
            # edge case
            masked_string = self.PAD_CHAR * self.block_size
            x = masked_string[:-1]
            y = masked_string[1:]
            return torch.tensor([self.stoi.get(c, 0) for c in x], dtype=torch.long), torch.tensor([self.stoi.get(c, 0) for c in y], dtype=torch.long)

        # 2. Break into 3 substrings: [prefix] [masked_content] [suffix]
        # Length of masked_content should be around 1/4 the length of truncated doc on average
        # We can implement it by drawing a length from a normal distribution or just random.randint where avg is 1/4
        # Since it says "has a chance of being more or less than 1/4", random.randint(1, max(1, trunc_len // 2)) works roughly.
        # Actually a simpler way is random.randint(0, trunc_len), but let's do something centered around trunc_len//4
        # Or even simpler: random.randint(0, trunc_len // 2) as long as average is around 1/4.
        mask_len = random.randint(0, trunc_len // 2)
        start_idx = random.randint(0, trunc_len - mask_len)

        prefix = doc[:start_idx]
        masked_content = doc[start_idx : start_idx + mask_len]
        suffix = doc[start_idx + mask_len:]

        # 3. Rearrange
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        
        # Pad to self.block_size
