...
\end{lstlisting}
    ...
    \ifans{
        The masks are used to set the previously computed attention scores $e_{t,i}$ of the padding tokens to $-\infty$ right before the softmax layer. Consequently, when the softmax function is applied, these extremely negative attention scores map precisely to an attention probability $\alpha_{t,i}$ of $0$. This ensures that when computing the final context vector $\ba_t$ via a weighted sum, the encoder hidden states corresponding to padding tokens contribute absolutely nothing to the result.
        It is necessary to use masks in this way because padding tokens are artificial fillers added solely to equalize sentence lengths within a batch and possess no real semantic meaning. If they were not masked out, the padding tokens would receive non-zero attention weights, injecting meaningless noise into the decoder's translation process.
    }
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{
        The model's corpus BLEU Score is 19.899.
    }
    ...
    \ifans{
        \begin{itemize}
            \item \textbf{Dot product vs Multiplicative:}
            \begin{itemize}
                \item \textbf{Advantage:} Dot product attention operates parameter-free without needing to learn an extra weight matrix $\bW$. This makes it computationally faster and highly space-efficient.
                \item \textbf{Disadvantage:} Dot product attention strictly requires the encoder hidden state $\bh_i$ and decoder hidden state $\bs_t$ to have the exact same dimensionality. Multiplicative attention avoids this limitation since the weight matrix $\bW$ can be sized to project representations of different dimensions.
            \end{itemize}
            \item \textbf{Additive vs Multiplicative:}
            \begin{itemize}
                \item \textbf{Advantage:} Additive attention tends to be more stable when the dimensionality is very large. In multiplicative attention, large dimensions can result in dot products with large variance, pushing the softmax function into regions with extremely small gradients. Additive attention also provides a non-linear mapping (via $\text{tanh}$), which can capture more complex relationships.
                \item \textbf{Disadvantage:} Additive attention is computationally more expensive and less memory-efficient. Multiplicative attention can be implemented via highly optimized, easily parallelizable matrix multiplications, whereas additive attention involves broadcasting, computing non-linearities, and learning multiple weight matrices ($\bW_1$, $\bW_2$, $\bv$).
            \end{itemize}
        \end{itemize}
    }
\end{parts}
