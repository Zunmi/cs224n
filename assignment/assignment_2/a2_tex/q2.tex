...
\begin{parts}
    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            {\Large \color{red}
            Momentum averages recent gradients, so the update direction depends less on the noise of any single minibatch. This reduces oscillation and makes the optimization trajectory smoother. Lower-variance updates help learning because the model can move more steadily toward good solutions and converge faster.
            }
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                {\Large \color{red}
                Parameters with smaller average gradient magnitudes will get larger updates, because their entries in $\bv$ are smaller and Adam divides by $\sqrt{\bv}$. Parameters with consistently large gradients therefore get smaller effective step sizes. This helps learning by adapting the step size to each parameter, preventing very large-gradient parameters from dominating while still letting slowly changing parameters make progress.
                }
                \end{subparts}
            ...
            \subpart[2]
                ...
                {\Large \color{red}
                For each coordinate $i$, we have $h_{\text{drop},i} = \gamma d_i h_i$, where
                $\mathbb{E}[d_i] = 1-p_{\text{drop}}$. Thus,
                \[
                \mathbb{E}[h_{\text{drop},i}] = \gamma\mathbb{E}[d_i]h_i
                = \gamma(1-p_{\text{drop}})h_i.
                \]
                Since the problem requires $\mathbb{E}[h_{\text{drop},i}] = h_i$, we must have
                \[
          ...
          \subpart[2] Why should dropout be applied during training? Why should dropout \textbf{NOT} be applied during evaluation? \textbf{Briefly explain in 2--4 sentences}. \textbf{Hint:} it may help to look at the dropout paper linked. \newline
                ...
                {\Large \color{red}
                During training, dropout acts as a regularizer: it prevents the network from relying too heavily on any one hidden unit and encourages the model to learn redundant, robust features that generalize better. During evaluation, we want deterministic and fully expressive predictions, so dropout should be turned off. If we kept dropout on at test time, predictions would become random and we would also be discarding model capacity when trying to measure final performance.
                }
