...
\begin{parts}
    ...
    {\color{red}$[$ROOT, presented, my]} & {\color{red}[findings, at, the, NLP, conference]} &  &  {\color{red}\texttt{SHIFT}}  \\
    {\color{red}$[$ROOT, presented, my, findings]} & {\color{red}[at, the, NLP, conference]} &  &  {\color{red}\texttt{SHIFT}}  \\
    {\color{red}$[$ROOT, presented, findings]} & {\color{red}[at, the, NLP, conference]} & {\color{red}findings$\to$my} &  {\color{red}\texttt{LEFT-ARC}}  \\
    {\color{red}$[$ROOT, presented]} & {\color{red}[at, the, NLP, conference]} & {\color{red}presented$\to$findings} &  {\color{red}\texttt{RIGHT-ARC}}  \\
    {\color{red}$[$ROOT, presented, at]} & {\color{red}[the, NLP, conference]} &  &  {\color{red}\texttt{SHIFT}}  \\
    {\color{red}$[$ROOT, presented, at, the]} & {\color{red}[NLP, conference]} &  &  {\color{red}\texttt{SHIFT}}  \\
    {\color{red}$[$ROOT, presented, at, the, NLP]} & {\color{red}[conference]} &  &  {\color{red}\texttt{SHIFT}}  \\
    {\color{red}$[$ROOT, presented, at, the, NLP, conference]} & {\color{red}[]} &  &  {\color{red}\texttt{SHIFT}}  \\
    {\color{red}$[$ROOT, presented, at, the, conference]} & {\color{red}[]} & {\color{red}conference$\to$NLP} &  {\color{red}\texttt{LEFT-ARC}}  \\
    {\color{red}$[$ROOT, presented, at, conference]} & {\color{red}[]} & {\color{red}conference$\to$the} &  {\color{red}\texttt{LEFT-ARC}}  \\
    {\color{red}$[$ROOT, presented, conference]} & {\color{red}[]} & {\color{red}conference$\to$at} &  {\color{red}\texttt{LEFT-ARC}}  \\
    {\color{red}$[$ROOT, presented]} & {\color{red}[]} & {\color{red}presented$\to$conference} &  {\color{red}\texttt{RIGHT-ARC}}  \\
    {\color{red}$[$ROOT]} & {\color{red}[]} & {\color{red}ROOT$\to$presented} &  {\color{red}\texttt{RIGHT-ARC}}  \\
    \end{tabular}\newline
    \end{adjustwidth}
    ...
    {\color{red}
    A sentence with $n$ words is parsed in \boxed{2n} steps. We need exactly $n$ \texttt{SHIFT}
    operations to move all $n$ words from the buffer onto the stack, and exactly $n$ arc operations
    (each \texttt{LEFT-ARC} or \texttt{RIGHT-ARC} removes one item from the stack) to reduce the stack
    from size $n+1$ back down to size $1$.
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        {\color{red}
        Let $z_i = (\bx\bW + \bb_1)_i = \sum_k x_k W_{ki} + b_{1,i}$. Then
        \[
        h_i = \relu(z_i) = \max(z_i, 0).
        \]
        Therefore,
        \[
        ...
        =
        \begin{cases}
        W_{ji} & \text{if } z_i > 0,\\[4pt]
        0 & \text{if } z_i < 0.
        \end{cases}
        \]
        Equivalently,
        \[
        \boxed{\frac{\partial h_i}{\partial x_j} = \mathbf{1}[z_i > 0] \, W_{ji}.}
        \]
        }
        ...
        {\color{red}
        Since $\byt = \smx(\bl)$ and
        \[
        CE(\by, \byt) = -\sum_{j=1}^3 y_j \log \hat y_j,
        \]
        the derivative with respect to the logit $l_i$ is
        \[
        \boxed{\frac{\partial CE(\by, \byt)}{\partial l_i} = \hat y_i - y_i.}
        \]
        Because the true class is $c$, this can also be written as
        \[
        \frac{\partial CE(\by, \byt)}{\partial l_i} =
        \begin{cases}
        \hat y_i - 1 & \text{if } i=c,\\[4pt]
        \hat y_i & \text{if } i\neq c.
        \end{cases}
        \]
        }
...
{\color{red} 
Using part-of-speech (POS) tags provides essential syntactic categories for words, which greatly benefits the parser in the following ways:
\begin{itemize}
    \item \textbf{Generalization against Data Sparsity}: POS tags abstract away from specific lexical items. If the parser encounters a rare or unseen word (Out-of-Vocabulary), its POS tag (e.g., Verb, Noun) still helps the parser infer its valid grammatical relations based on structural rules it learned from common words.
    \item \textbf{Disambiguation}: Many words have multiple meanings and can act as different parts of speech depending on the context (e.g., "book" as a noun vs. a verb). Explicit POS tags give the parser a direct signal about the word's grammatical role in this specific sentence without forcing the parser to infer it purely from word embeddings.
\end{itemize}
}
