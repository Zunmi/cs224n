...
{\color{red}
Since $\by$ is a one-hot vector for the true outside word $o$, we have $y_o = 1$
and $y_w = 0$ for all $w \neq o$. Therefore,
\[
-\sum_{w \in \text{Vocab}} y_w \log(\hat y_w)
= -\left(y_o\log(\hat y_o) + \sum_{w \neq o} y_w\log(\hat y_w)\right)
= -(1\cdot\log(\hat y_o) + 0)
= -\log(\hat y_o).
\]
Hence the naive-softmax loss is exactly the cross-entropy loss.
}
...
\begin{enumerate}[label=\roman*.]
    ...
    = -\log\left(\frac{e^{\bu_o^\top \bv_c}}{\sum_{w \in \text{Vocab}} e^{\bu_w^\top \bv_c}}\right)
    = -\bu_o^\top \bv_c + \log\left(\sum_{w \in \text{Vocab}} e^{\bu_w^\top \bv_c}\right).
    \]
    Differentiating with respect to $\bv_c$ gives
    \[
    \frac{\partial \bJ}{\partial \bv_c}
    = -\bu_o + \frac{1}{\sum_w e^{\bu_w^\top \bv_c}} \sum_w e^{\bu_w^\top \bv_c}\bu_w
    = -\bu_o + \sum_w \hat y_w \bu_w.
    \]
    Since $\bU\by = \bu_o$ and $\bU\hat{\by} = \sum_w \hat y_w\bu_w$, we obtain
    \[
    \boxed{\frac{\partial \bJ_{\text{naive-softmax}}(\bv_c, o, \bU)}{\partial \bv_c}
    = \bU\hat{\by} - \bU\by = \bU(\hat{\by} - \by).}
    \]
    }
    \item
    ...
    \[
    \boxed{\bU(\hat{\by} - \by) = \mathbf{0}.}
    \]
    In particular, $\hat{\by} = \by$ is a sufficient condition.
    }
    \item
    ...
    \[
    \bv_c \leftarrow \bv_c - \alpha \frac{\partial \bJ}{\partial \bv_c}
    = \bv_c - \alpha\bU(\hat{\by} - \by)
    = \bv_c + \alpha\bU\by - \alpha\bU\hat{\by}.
    \]
    The term $\bU\by = \bu_o$ moves $\bv_c$ toward the true outside word vector,
    increasing the compatibility between the center word and the correct outside word.
    The term $\bU\hat{\by}$ is the model's predicted average outside-word vector, so
    subtracting it pushes $\bv_c$ away from words that the model currently predicts too
    strongly, especially incorrect ones. Together, these terms move $\bv_c$ closer to the
    correct outside word and away from wrongly predicted outside words.
    }
...
{\color{red}
L2 normalization removes magnitude information and keeps only direction. If
$\mathbf{u}_x = \alpha\mathbf{u}_y$ for some $\alpha > 0$, then
\[
\frac{\mathbf{u}_x}{\|\mathbf{u}_x\|_2} = \frac{\mathbf{u}_y}{\|\mathbf{u}_y\|_2}.
\]
Therefore, normalization takes away useful information when the length of an
embedding carries task-relevant information, such as sentiment strength or word
importance. In that case, two words with the same direction but different
magnitudes become indistinguishable after normalization. On the other hand,
normalization does not hurt when only the direction of the embedding matters for
the downstream task and the magnitude contains no useful information.
}
% Question 1-D
\part[1]
...
\[
\bJ_{\text{naive-softmax}}(\bv_c, o, \bU)
= -\log\left(\frac{e^{\bu_o^\top \bv_c}}{\sum_j e^{\bu_j^\top \bv_c}}\right)
= -\bu_o^\top \bv_c + \log\left(\sum_j e^{\bu_j^\top \bv_c}\right).
\]
Now differentiate with respect to a particular outside word vector $\bu_w$.
For the first term,
\[
\frac{\partial}{\partial \bu_w}(-\bu_o^\top \bv_c)
=
\begin{cases}
-\bv_c & \text{if } w=o,\\[4pt]
0 & \text{if } w\neq o,
...
\[
Z = \sum_j e^{\bu_j^\top \bv_c}.
\]
Then
\[
\frac{\partial}{\partial \bu_w}\log Z
= \frac{1}{Z}\frac{\partial Z}{\partial \bu_w}
= \frac{1}{Z}\frac{\partial}{\partial \bu_w}\left(\sum_j e^{\bu_j^\top \bv_c}\right).
\]
Only the $j=w$ term depends on $\bu_w$, so
\[
\frac{\partial}{\partial \bu_w}\log Z
= \frac{1}{Z} e^{\bu_w^\top \bv_c}\bv_c
= \frac{e^{\bu_w^\top \bv_c}}{\sum_j e^{\bu_j^\top \bv_c}}\bv_c
= \hat y_w \bv_c.
\]
Combining the two parts,
\[
\frac{\partial \bJ_{\text{naive-softmax}}(\bv_c, o, \bU)}{\partial \bu_w}
= -y_w\bv_c + \hat y_w\bv_c
= (\hat y_w - y_w)\bv_c.
\]
Therefore,
\[
...
\begin{cases}
(\hat y_o - 1)\bv_c & \text{if } w=o,\\[4pt]
\hat y_w\bv_c & \text{if } w\neq o.
\end{cases}
\]
